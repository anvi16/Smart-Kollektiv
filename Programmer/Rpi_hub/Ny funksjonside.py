...
@author: magnu
"""
import kommunikasjon as k      # Imports the self created script kommunikasjon
import time
#import MQTT_PY as k
import json
import enum
import Power_usage as p
import pandas as pd
...
def kitchen_status(room, time, user):     # Function for kitchen booking
    ...
    if room == 'Kitchen':        # Checks if the recived message is Kitchen
        ...
        if kitchen['slot_1'] == 'free':
            ...
            k.mqtt_publish("booked", "You have booked kitchen slot 1 for 30 minutes")         
            ...
            k.mqtt_publish("booked", "You have booked kitchen slot 2 for 30 minutes")      
            ...
            if rem_1 <= rem_2:
                k.mqtt_publish("booked", "The kitchen is full, it will be free in " + str(round(rem_1/60000, 0)) + " minutes")
            elif rem_2 <= rem_1:
                k.mqtt_publish("booked","The kitchen is full, it will be free in " + str(round(rem_2/60000, 0)) + " minutes")                       
        ...
        k.mqtt_publish('booked', 'You have canceled your slot')
...
def bathroom_status(room, time, user):                               # Function for bathroom booking
    ...
    if room == 'Toilet':                                 # Checks if message asks to book the toilet
        ...
        if ('free' in bathroom.values()) == True:   # Cheks if dictionary bathroom 
            ...
            k.mqtt_publish('booked', 'You have booked the bathroom for 5 minutes')
            ...
            if rem_bath_1 <= rem_bath_2:
                k.mqtt_publish('booked', 'The bathroom is full, it will be free in ' + str(round(rem_bath_1/60000, 0)) + ' minutes')
            elif rem_bath_1 >= rem_bath_2:
                k.mqtt_publish('booked', 'The bathroom is full, it will be free in ' + str(round(rem_bath_2/60000, 0)) + ' minutes')
        ...
        if ('free' in bathroom.values()) == True:   # Cheks if dictionary bathroom if current_milli_time() >= kit_time1 + 1800000
            ...
            k.mqtt_publish('booked', 'You have booked the bathroom for 15 minutes')
...
def livingroom_status(room, time, user):              # Function for livingroom booking
    ...
    if room == 'Livingroom':                    # Checks if subscribed value is Livingroom
        ...
        if livingroom['slot_1'] == 'free':
            ...
            k.mqtt_publish('booked', 'You have booked livingroom slot 1 for ' + str(round(time/60000, 0)) + ' minutes')
            # Publishes to mqtt
            ...
            k.mqtt_publish('booked', 'You have booked livingroom slot 2 for ' + str(round(time/60000, 0)) + ' minutes')
            # Publishes to mqtt
            ...
            k.mqtt_publish('booked', 'You have booked livingroom slot 3 for ' + str(round(time/60000, 0)) + ' minutes')
            # Publishes to mqtt
            ...
            if (rem_1 <= rem_2 and rem_1 <= rem_3):
                k.mqtt_publish('booked', "The livingroom is full, it will be free in " + str(round(rem_1/60000, 0)) + " minutes")
            elif (rem_2 <= rem_1 and rem_2 <= rem_3):
                k.mqtt_publish('booked', "The livingroom is full, it will be free in " + str(round(rem_2/60000, 0)) + " minutes")
            elif (rem_3 <= rem_1 and rem_3 <= rem_1):
                k.mqtt_publish('booked', "The livingroom is full, it will be free in " + str(round(rem_3/60000, 0)) + " minutes")            
...
def on_message(client, userdata, msg):  # Function for subscribing to a topic in mqtt broker  
    msg.payload.decode("utf-8")
    payload = json.loads(msg.payload)
    print(payload)
    user = payload['id']
    if (payload['room'] == Room.Kitchen):
    ...
    elif (payload['room'] == Room.Toilet):
        time = 5*60000                                  # Sets time for use of toilet
        bathroom = bathroom_status('Toilet', time, user)      # calls bathroom function
        
    elif (payload['room'] == Room.Shower):
        time = 15*60000
        bathroom = bathroom_status('Shower', time, user)
    
    elif (payload['room'] == Room.Liv30):
        time = 30*60000
        livingroom = livingroom_status('Livingroom', time, user)
        
    elif (payload['room'] == Room.Liv60):
        time = 60*60000
        livingroom = livingroom_status('Livingroom', time, user)
        
    elif (payload['room'] == Room.Liv120):
        time = 120*60000
        livingroom = livingroom_status('Livingroom', time, user)        
        
    elif (payload['room'] == Room.CanKit1):
        kitchen = kitchen_status('Cancel kit slot1', 0)
        
    elif (payload['room'] == Room.CanKit2):
        kitchen = kitchen_status('Cancel kit slot2', 0)
    
    elif (payload['room'] == Room.CanBath):
        bathroom = bathroom_status('Cancel bathroom', 0)
    
    elif (payload['room'] == Room.CanLiv1):
        livingroom = livingroom_status('Cancel liv slot1', 0)
    
    elif (payload['room'] == Room.CanLiv2):
        livingroom = livingroom_status('Cancel liv slot2', 0)
    
    elif (payload['room'] == Room.CanLiv3):
        livingroom = livingroom_status('Cancel liv slot3', 0)
        
        

def on_publish(client, userdata, result): # Function making it possible to publish to a topic
    print('data published \n')
    pass



k.mqtt_loop(on_message, on_publish) # Make the program loop the connection for subscribe and publish
